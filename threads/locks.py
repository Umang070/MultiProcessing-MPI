...
def add_subtract_withLock():
    ...
    adder_thread = Thread(target=adder_WithLock, args=(10, 10000000, lock))
    adder_thread.start()
    # start a thread making subtractions
    subtractor_thread = Thread(target=subtractor_WithLock, args=(10, 10000000, lock))
...
def add_subtract():
    # start a thread making additions
    adder_thread = Thread(target=adder, args=(10, 10000000))
    adder_thread.start()
    # start a thread making subtractions
    subtractor_thread = Thread(target=subtractor, args=(10, 10000000))
    subtractor_thread.start()
    # wait for both threads to finish
    print('Waiting for threads to finish...')
    adder_thread.join()
